...
class ContactAwareGraspLiftEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        obs_dim = 26 if self.env_config.observation_mode == "baseline" else 28
    ...
    def _build_mjcf(self) -> str:
        object_half_extents = " ".join(f"{value:.5f}" for value in self.env_config.object_half_extents)
        object_friction = " ".join(f"{value:.5f}" for value in self.env_config.object_friction)
        finger_friction = " ".join(f"{value:.5f}" for value in self.env_config.finger_friction)
    ...
    def set_manual_configuration(
        ...
        *,
        ...
        if isinstance(finger_positions, (float, int)):
            left_finger = right_finger = float(finger_positions)
        else:
            left_finger = float(finger_positions[0])
            right_finger = float(finger_positions[1])

        object_quat = (
            np.asarray(object_quat, dtype=np.float64)
        ...
        self.data.qpos[self._object_qpos_adr + 3 : self._object_qpos_adr + 7] = object_quat
        if zero_vel:
            self.data.qvel[:] = 0.0
